...
public class Decay extends SmithPApplet {
	/**
	 * 
	 */
	...
	public void draw() {
		...
		if (tracking) {
			...
			if (userMap_still == null) {
				userMap_still = kinect.userMap();
				userMap = kinect.userMap();
			} else {
				userMap = userMap_still;
			...
			for (int petal_id = 0; petal_id < n_petals; petal_id++) {
				...
				if (isTouching) {
					...
					/*
					 * Xs.remove(petal_id); Ys.remove(petal_id);
					 */

					// removing the touched petal and putting new petal
					Xs.set(petal_id, randomize(0, width / 2));
					Ys.set(petal_id, 0);
			...
			for (int i = 0; i < userMap.length; i++) {
				...
				for (int tpetal_id = 0; tpetal_id < n_touchedPetals; tpetal_id++) {
					...
					if ((bodyX - petalX) * (bodyX - petalX)
							+ (bodyY - petalY) * (bodyY - petalY) < radius* radius) {
						userMap[getLoc(bodyX, bodyY)] = 0;
					}
				}
