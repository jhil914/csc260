...
public class growingtree extends SmithPApplet{
	...
	public void draw() {
		...
		if(mousePressed){
			...
			for (int i = 1; i < myLeaves.length; i++) {
			    ...
			    myLeaves[i][j].x = xpos[i]+(int) random(-2,2);
			    myLeaves[i][j].y = ypos[i]+(int) random(-2,2);
			    myLeaves[i][j].draw();
				}
				else{
	...
	/**   if(x>200){
	   ...
	   if(range>0){
		   x=x-(int) (noiseval*5);
			  y = (int) (noiseval*80+100);
	   }
	   else if(range<0){
		   x = x+(int) (noiseval*5); //this updates the position of the point and makes it move across the screen
		    y = (int) (noiseval*80+100);
	   }
